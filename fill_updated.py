...
def load_playlists(spotify_api_dict):
    ...
    for i in range(len(spotify_api_dict['playlist_info'])):
        playlist_id = spotify_api_dict['playlist_info'][i]['playlist_id']
        if db.session.query(Playlist).filter_by(playlist_id=playlist_id).scalar() is not None:
            pass
        else:
            playlist_name = spotify_api_dict['playlist_info'][i]['playlist_name']
...
def load_tracks(spotify_api_dict):
    ...
    for i in range(len(spotify_api_dict['album_info'])):
        for i in range(len(spotify_api_dict['album_info'][i]['album_tracks_uris'])):
            album_track_uri = spotify_api_dict['album_info'][i]['album_tracks_uris'][i]
